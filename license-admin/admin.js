...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setTimeout(loadLicenseHistory, 1000);
...
document.getElementById('refreshHistoryBtn').addEventListener('click', loadLicenseHistory);
...
async function loadLicenseHistory() {
  try {
    ...
    const historyDiv = document.getElementById('licenseHistory');
    
    if (licenses.length === 0) {
      historyDiv.innerHTML = '<div class="empty-state">Nenhuma licença gerada ainda.</div>';
      return;
    }
    
    licenses.sort((a, b) => new Date(b.issuedAt) - new Date(a.issuedAt));
    
    historyDiv.innerHTML = licenses.map(lic => `
      <div class="history-item">
        <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:8px">
          <strong>${lic.holder}</strong>
          <span style="font-size:.75rem;color:#6b7280">${lic.generatedAt}</span>
        </div>
        <div class="license-key">${lic.license}</div>
        <div class="meta">
          <span>📄 ${lic.document}</span>
          <span>⏳ ${lic.days} dias</span>
          <span>📅 Expira: ${lic.expiresAtFormatted}</span>
        </div>
      </div>
    `).join('');
    ...
    document.getElementById('licenseHistory').innerHTML = 
